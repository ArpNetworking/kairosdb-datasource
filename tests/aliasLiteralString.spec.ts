...
describe('Literal String Alias Issue', () => {
  test('should demonstrate the problem with literal string aliases', () => {
    ...
    const variableResults = individualScopedVars.map((vars) => templateSrv.replace(variableAlias, vars));

    const curlyResults = individualScopedVars.map((vars) => templateSrv.replace(curlyVariableAlias, vars));
    ...
    expect(variableResults).toEqual(['Attic', 'Bedroom', 'Garage', 'Living Room', 'Office']);

    // Alternative solution: With curly braces
    expect(curlyResults).toEqual(['{Attic}', '{Bedroom}', '{Garage}', '{Living Room}', '{Office}']);
  });
