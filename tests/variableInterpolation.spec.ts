...
describe('Template Variable Interpolation', () => {
  ...
  describe('Basic Variable Interpolation', () => {
    test('should interpolate simple $variable syntax', () => {
      ...
      expect(datasource.interpolateVariable('system.$metric.host.$server', scopedVars)).toBe(
        'system.cpu.usage.host.web01'
      );
    });
    ...
    test('should handle multi-value variables with single selection', () => {
      ...
      expect(datasource.interpolateVariable('host.$servers', scopedVars)).toBe('host.web01');
    });
    ...
    test('should handle multi-value variables with multiple selections', () => {
      ...
      expect(datasource.interpolateVariable('host.$servers', scopedVars)).toBe('host.web01,web02,web03');
    });
    ...
    test('should handle All option (*)', () => {
      ...
      expect(datasource.interpolateVariable('host.$servers', scopedVars)).toBe('host.*');
    });
    ...
    test('should leave unresolved variables unchanged', () => {
      ...
      expect(datasource.interpolateVariable('host.$server.metric.$unknown', scopedVars)).toBe(
        'host.web01.metric.$unknown'
      );
    });
    ...
    test('should handle empty variables gracefully', () => {
      ...
      expect(datasource.interpolateVariable('metric.$empty.value', scopedVars)).toBe('metric..value');

      expect(datasource.interpolateVariable('metric.$nullvar.value', scopedVars)).toBe('metric.null.value');
    });
  });
  ...
  describe('Variable Query Interpolation', () => {
    test('should interpolate variables in metrics() queries', async () => {
      ...
      jest
        .spyOn(datasource, 'getMetricNames')
        .mockResolvedValue(['system.cpu.usage', 'system.memory.usage', 'system.disk.usage']);
    ...
    test('should interpolate variables in tag_values() queries', async () => {
      ...
      const result = await datasource.metricFindQuery('tag_values($metric, $tag_name, datacenter=$datacenter)', {
        scopedVars,
      });
  ...
  describe('Advanced Variable Formats', () => {
    test('should handle regex format ${variable:regex}', () => {
      ...
      expect(datasource.interpolateVariable('${servers}', scopedVars)).toBe('web01,web02,web03');
    });
  ...
  describe('Edge Cases and Error Handling', () => {
    test('should handle nested variable references', () => {
      ...
      expect(datasource.interpolateVariable('${${env}_prefix}.cpu', scopedVars)).toBe('${${env}_prefix}.cpu'); // Current implementation limitation - no nested resolution
    });
    ...
    test('should handle variables with special characters', () => {
      ...
      expect(datasource.interpolateVariable('$metric-name.$server_01', scopedVars)).toBe('$metric-name.web01'); // Limitation: doesn't handle hyphens
    });
    ...
    test('should handle circular references gracefully', () => {
      ...
      expect(datasource.interpolateVariable('$var1', scopedVars)).toBe('$var2'); // Doesn't expand further (good!)
    });

    test('should handle undefined or null scopedVars', () => {
      expect(datasource.interpolateVariable('$test', undefined)).toBe('$test');

      expect(datasource.interpolateVariable('$test', null as any)).toBe('$test');
    });

    test('should handle empty or undefined values', () => {
      expect(datasource.interpolateVariable('', { test: { text: 'value', value: 'value' } })).toBe('');

      expect(datasource.interpolateVariable(null as any, { test: { text: 'value', value: 'value' } })).toBe(null);
    });
  });
});
