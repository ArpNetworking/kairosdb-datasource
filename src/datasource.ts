...
import { escapeLiteralBraces, unescapeLiteralBraces, hasEscapedBraces } from './utils/tagValueEscaping';
...
export class DataSource extends DataSourceApi<KairosDBQuery, KairosDBDataSourceOptions> {
        ...
        query: target.query
          ? {
              ...
              groupBy: target.query.groupBy
                ...
                : target.query.groupBy,
              aggregators: target.query.aggregators
                ? target.query.aggregators.map((agg) => ({
                    ...agg,
                    parameters: agg.parameters
                      ? agg.parameters.map((param) => ({
                          ...param,
      ...
      validTargets.forEach((target, targetIndex) => {
        ...
        expansion.names.forEach((metricName, index) => {
          ...
          if (query.aggregators && query.aggregators.length > 0) {
            ...
            metric.aggregators = visibleAggregators.map((agg) => {
              ...
              if (agg.parameters && agg.parameters.length > 0) {
                ...
                const samplingParams = agg.parameters.filter(
                  (p) => p.type === 'sampling' || p.type === 'sampling_unit'
                );
                const otherParams = agg.parameters.filter(
                  (p) => p.type !== 'alignment' && p.type !== 'sampling' && p.type !== 'sampling_unit'
                );

                // Handle alignment parameter first
                if (
                  alignmentParam &&
                  alignmentParam.value !== undefined &&
                  alignmentParam.value !== null &&
                  alignmentParam.value !== ''
                ) {
                      ...
                      if (samplingParams.length > 0) {
                        ...
                        samplingParams.forEach((param) => {
                          if (param.value !== undefined && param.value !== null && param.value !== '') {
                            ...
                            if (
                              parameterBuilder.isOverriddenByAutoValue &&
                              parameterBuilder.isOverriddenByAutoValue(param)
                            ) {
                              processedValue =
                                param.type === 'sampling'
                                  ? parameterBuilder.autoIntervalValue
                                  : parameterBuilder.autoIntervalUnit;
                            }

                            // Convert numeric values
                            if (param.name === 'value') {
                              processedValue =
                                typeof processedValue === 'string' ? parseFloat(processedValue) : processedValue;
                            }
          ...
          if (query.groupBy) {
            ...
            if (query.groupBy.value && !Array.isArray(query.groupBy.value) && query.groupBy.value.range_size) {
              metric.group_by.push({
                name: 'value',
      ...
      data.queries.forEach((query, responseQueryIndex: number) => {
        ...
        query.results.forEach((result, resultIndex: number) => {
          ...
          if (this.isHistogramData(result.values[0])) {
            ...
            const histogramFrames = this.parseHistogramData(result, seriesName, targetForQuery.refId, samplingInterval);
      ...
      if (error && typeof error === 'object') {
        ...
        if (httpError.status) {
          ...
          switch (httpError.status) {
            case 502:
              userMessage =
                'KairosDB server is unreachable (502 Bad Gateway). Please check if KairosDB is running and accessible.';
              break;
            case 500:
      ...
      if (Array.isArray(tagValues)) {
        const processedValues = tagValues
          .map((value) => {
            ...
            // Check if this value had escaped braces (contains our placeholders)
            // If so, it's a literal value and should NOT be split
            if (hasEscapedBraces(interpolated)) {
              // Restore the literal braces and return as single value
      ...
      if (Array.isArray(tagValues)) {
        const processedValues = tagValues
          .map((value) => {
            ...
            // e.g., "foo-{value1,value2}" should become ["foo-value1", "foo-value2"]
            // Also handles "{value1,value2}-bar" and "foo-{value1,value2}-bar"
            // This pattern only applies to actual template variable results, not literal braces
            const prefixMatch = interpolated.match(/^(.*?)\{(.+)\}(.*)$/);
            if (prefixMatch) {
              const prefix = prefixMatch[1] || '';
              const values = prefixMatch[2];
   ...
   */
  getMetricNamesCacheStats(): {
    metricNames: { size: number; keys: string[] };
    apiResponses: { size: number; keys: string[] };
  } {
    return {
   ...
   */
  private parseHistogramData(result: any, seriesName: string, targetRefId: string, samplingInterval: number): any[] {
    const dataFrames: any[] = [];
    ...
    for (const datapoint of result.values) {
      ...
      const sortedBinKeys = Object.keys(bins).sort((a, b) => parseFloat(a) - parseFloat(b));
  ...
  private getSnapToInterval(grafanaInterval: string): string {
    const snapToIntervals =
      this.settings.jsonData.snapToIntervals || '1m,5m,10m,15m,30m,1h,2h,3h,4h,6h,12h,1d,2d,3d,7d';
  ...
  private requiresSamplingParameters(aggregatorName: string): boolean {
    ...
    const hasSamplingParams = aggregatorDef.parameters?.some(
      (param) => param.type === 'sampling' || param.type === 'sampling_unit'
    );
  ...
  private expandMetricNames(
    ...
    refId: string,
    scopedVars?: ScopedVars
  ): { names: string[]; variableValues: ScopedVars[] } {
